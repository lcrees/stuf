    ...
    def _populate(self, iterable):
        ...
        for k, v in iterable.items():
            if isinstance(v, Sequence):
                ...
                trial = new(self.default_factory, iterable)
                if len(trial) > 0:
                    self[k] = trial
                else:
                    self[k] = v
...
#
#    @classmethod
#    def _new(cls, factory=None, fargs=(), fkw=None, **kw):
#        '''
...
class orderedstuf(writestuf, OrderedDict):
...
#            key = curr[2]
#            value = self[key]
        for key, value in super(orderedstuf, self).__iter__():
